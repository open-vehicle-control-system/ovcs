...
compose/local/
  ...
  simulation/             the simulator — baked into the image
...
docker compose -f simulation.yml up -d          # the simulator
docker compose -f simulation.yml logs -f sim    # ^C once it settles
```

Every command below is run from `compose/local/`. The stacks are named
files rather than a `docker-compose.yml` per directory, so `-f` is
always spelled out.
...
|---|---|---|---|
| gamepad | `docker compose -f simulation.yml --profile teleop up -d teleop` | drive it with a stick instead of `topic pub` | a joystick at `/dev/input/js0` |
...
`docker compose -f simulation.yml stop teleop`, first. (Nav2 uses a separate topic and
does not collide.)

Tear down with `docker compose -f simulation.yml down`, plus
`docker compose -f base.yml rm -sf zenohd`.

## The GUI and the gamepad are separate services

Deliberately, so the simulation itself needs neither a display nor a
gamepad — `docker compose -f simulation.yml up -d` works on a headless machine, which is
