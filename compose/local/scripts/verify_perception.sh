...
cleanup() {
  ...
  if [ "$KEEP_UP" = "1" ]; then
    ...
    printf '  stop with: (cd %s && docker compose -f simulation.yml down && docker compose -f base.yml rm -sf ros2 zenohd)\n' \
      "$LOCAL"
    return $code
  fi
  log "Tearing down"
  ...
  (cd "$LOCAL" && timeout 120 docker compose -f simulation.yml down >/dev/null 2>&1)
  (cd "$LOCAL" && timeout 120 docker compose -f base.yml rm -sf ros2 zenohd >/dev/null 2>&1)
  return $code
}
trap cleanup EXIT
...
(cd "$LOCAL" && ZENOH_ENDPOINT_IP=127.0.0.1 timeout 180 docker compose -f base.yml \
...
(cd "$LOCAL" && timeout 300 docker compose -f simulation.yml up -d) || { fail "sim failed to start"; exit 1; }
...
(cd "$LOCAL" && timeout 300 docker compose -f base.yml exec -T ros2 bash -lc \
  'source /opt/ros/*/setup.bash 2>/dev/null; exec python3 -') \
  < "$LOCAL/simulation/scripts/perception_test.py"
result=$?
