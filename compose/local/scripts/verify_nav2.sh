...
# arrives. See ../simulation/scripts/nav2_test.py.
#
# Usage:
...
cleanup() {
  ...
  if [ "$KEEP_UP" = "1" ]; then
    ...
    printf '  stop with: (cd %s && docker compose -f simulation.yml --profile nav2 down && docker compose -f base.yml rm -sf ros2 zenohd)\n' \
      "$LOCAL"
    return $code
  fi
  log "Tearing down"
  (cd "$LOCAL" && timeout 120 docker compose -f simulation.yml --profile nav2 down >/dev/null 2>&1)
...
(cd "$LOCAL" && ZENOH_ENDPOINT_IP=127.0.0.1 timeout 300 docker compose -f simulation.yml up -d) \
...
(timeout 300 docker exec -i ovcs-nav2 bash -lc \
  ...
  < "$LOCAL/simulation/scripts/nav2_test.py"
result=$?
