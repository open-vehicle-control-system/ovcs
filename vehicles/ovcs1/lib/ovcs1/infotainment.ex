defmodule Ovcs1.Infotainment do
  ...
  @status_frame_names [
    "vms_status",
    "front_controller_alive",
    "controls_controller_alive",
    "rear_controller_alive"
  ]

  @impl true
  def init(_) do
    :ok =
      Emitter.configure(:ovcs, "infotainment_status", %{
  ...
  def handle_info({:handle_frame, %Frame{name: "gear_status", signals: signals}}, state) do
    ...
    state =
      if selected_gear != state.requested_gear &&
           Time.diff(Time.utc_now(), state.last_gear_update_at) > @gear_selection_delay do
        :ok =
          Emitter.update(:ovcs, "infotainment_status", fn data ->
  ...
  def handle_info({:handle_frame, %Frame{name: "vms_status", signals: signals}}, state) do
    ...
    {:noreply, %{state | ready_to_drive: ready_to_drive, vms_status: status}}
  end

  def handle_info(
        {:handle_frame, %Frame{name: "front_controller_alive", signals: signals}},
        state
      ) do
  ...
  def handle_info(
        {:handle_frame, %Frame{name: "controls_controller_alive", signals: signals}},
        state
      ) do
    %{"status" => %Signal{value: status}} = signals
  ...
  def handle_info(
        {:handle_frame, %Frame{name: "passenger_compartment_status", signals: signals}},
        state
      ) do
    %{
  ...
  def handle_info(
        {:handle_frame,
         %Frame{name: "rear_controller_digital_and_analog_pin_status", signals: signals}},
        state
      ) do
    %{
  ...
  def handle_info(
        {:handle_frame, %Frame{name: "twelve_volt_battery_status", signals: signals}},
        state
      ) do
  ...
  defp compute_components_statuses(state) do
    ...
    {:ok, front_controler_is_alive} =
      ReceivedFrameWatcher.is_alive?(:ovcs, "front_controller_alive")

    {:ok, controls_controller_is_alive} =
      ReceivedFrameWatcher.is_alive?(:ovcs, "controls_controller_alive")

    {:ok, rear_controller_is_alive} =
      ReceivedFrameWatcher.is_alive?(:ovcs, "rear_controller_alive")
    ...
    %{
      ...
      | vms_computed_status:
        ...
        front_controler_computed_status:
          if front_controler_is_alive do
            state.front_controller_status
          else
            "MISSING"
          end,
        controls_controller_computed_status:
          if controls_controller_is_alive do
            state.controls_controller_status
          else
            "MISSING"
          end,
        rear_controller_computed_status:
          if rear_controller_is_alive do
            state.rear_controller_status
          else
            "MISSING"
          end
    }
