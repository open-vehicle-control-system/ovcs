defmodule Ovcs1.Vms.OVCSCANForwarder do
  @moduledoc """
  ...
  @impl true
  def init(%{
        passenger_compartement_source: passenger_compartment_source,
  ...
  def handle_info(
        %Bus.Message{name: :front_left_door_open, value: front_left_door_open, source: source},
        state
      )
      when source == state.passenger_compartment_source do
    {:noreply, %{state | front_left_door_open: front_left_door_open}}
  end

  def handle_info(
        %Bus.Message{name: :front_right_door_open, value: front_right_door_open, source: source},
        state
      )
      when source == state.passenger_compartment_source do
  ...
  def handle_info(
        %Bus.Message{name: :rear_left_door_open, value: rear_left_door_open, source: source},
        state
      )
      when source == state.passenger_compartment_source do
  ...
  def handle_info(
        %Bus.Message{name: :rear_right_door_open, value: rear_right_door_open, source: source},
        state
      )
      when source == state.passenger_compartment_source do
  ...
  def handle_info(
        %Bus.Message{name: :trunk_door_open, value: trunk_door_open, source: source},
        state
      )
      when source == state.passenger_compartment_source do
  ...
  def handle_info(%Bus.Message{name: :beam_active, value: beam_active, source: source}, state)
      when source == state.passenger_compartment_source do
    {:noreply, %{state | beam_active: beam_active}}
  end

  def handle_info(
        %Bus.Message{name: :handbrake_engaged, value: handbrake_engaged, source: source},
        state
      )
      when source == state.passenger_compartment_source do
  ...
  defp adapt_compartment_status(state, key, emitted_key) do
    ...
    case state[emitted_key] == new_value do
      ...
      false ->
        :ok =
          Emitter.update(:ovcs, "passenger_compartment_status", fn data ->
            %{data | "#{key}" => new_value}
          end)
