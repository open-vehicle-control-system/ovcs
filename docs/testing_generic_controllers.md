...
The steering servo is on HAT output 0 (`0x705`) at 100 Hz. Use the HAT
firmware with per-output prescaler selection and nearest-tick rounding:
at 100 Hz it runs a /10 prescaler at 64 MHz, 0.15625 us per tick, where
a fixed /100 prescaler leaves the servo only coarse pulse steps.
Updating only the VMS does not change that hardware quantisation.
