defmodule VmsCore.Status do
  ...
  def init(%{
    ...
    :ok =
      Emitter.configure(:ovcs, "vms_status", %{
    ...
    :ok =
      Emitter.configure(:ovcs, "vms_command", %{
  ...
  def handle_info(%Bus.Message{name: :vms_status, value: vms_status, source: source}, state)
      when source == state.vms_status_source do
  ...
  defp update_vms_status(state) do
    cond do
      state.resetting && state.emitted_vms_status != "RESETTING" ->
        :ok =
          Emitter.update(:ovcs, "vms_status", fn data ->
  ...
  defp emit_vms_command_if_required(state) do
    cond do
      state.resetting && !state.reset_generic_controllers_frame_enabled ->
        :ok =
          Emitter.update(:ovcs, "vms_command", fn data ->
