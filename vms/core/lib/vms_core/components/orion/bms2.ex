defmodule VmsCore.Components.Orion.Bms2 do
  ...
  def handle_info({:handle_frame, %Frame{name: "bms_status_2", signals: signals}}, state) do
    ...
    {:noreply,
     %{
       state
       | # Add one volt to compensate the internal BMS' diode
         twelve_volt_battery_voltage: D.add(twelve_volt_battery_voltage, @one),
  ...
  defp emit_metrics(state) do
    ...
    Bus.broadcast("messages", %Bus.Message{
      name: :pack_voltage,
      value: state.pack_voltage,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :pack_state_of_charge,
      value: state.pack_state_of_charge,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :j1772_plug_state,
      value: state.j1772_plug_state,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :twelve_volt_battery_voltage,
      value: state.twelve_volt_battery_voltage,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :pack_lowest_temperature,
      value: state.pack_lowest_temperature,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :pack_highest_temperature,
      value: state.pack_highest_temperature,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :pack_average_temperature,
      value: state.pack_average_temperature,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :is_charging_source_enabled,
      value: state.is_charging_source_enabled,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :is_ready_source_enabled,
      value: state.is_ready_source_enabled,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :charger_safety_relay_enabled,
      value: state.charger_safety_relay_enabled,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :discharge_relay_enabled,
      value: state.discharge_relay_enabled,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :charge_interlock_enabled,
      value: state.charge_interlock_enabled,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :balancing_active,
      value: state.balancing_active,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :bms_error,
      value: state.bms_error,
      source: __MODULE__
    })
