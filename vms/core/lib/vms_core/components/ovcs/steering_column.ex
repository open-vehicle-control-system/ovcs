defmodule VmsCore.Components.OVCS.SteeringColumn do
  ...
  @ki D.new("2")
  @kd D.new("0.004")
  @minimum_steering_angle D.new("-300")
  @maximum_steering_angle D.new("300")
    ...
    case state.automatic_mode_enabled do
      true ->
        if state.angle |> D.lt?(@maximum_steering_angle) && state.angle |> D.gt?(@minimum_steering_angle) do
          ...
          IO.inspect(target_motor_speed_percentage, label: "target")
  ...
  @impl true
  def handle_call({:test_set_angle, angle}, _from, state) do
    {:reply, :ok, %{state | enable_automatic_mode: true, desired_angle: angle}}
  end
  def handle_call(:deactivate, _from, state) do
    {:reply, :ok, %{state | enable_automatic_mode: false, frequency: 0}}
  end

  def test_set_angle(angle) do
    #TODO set min/max
    GenServer.call(__MODULE__, {:test_set_angle, angle})
  end
