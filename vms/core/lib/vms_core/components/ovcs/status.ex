defmodule VmsCore.Components.OVCS.Status do
  ...
  def init(%{bms_status_source: bms_status_source}) do
    :ok =
      Emitter.configure(:ovcs, "pack_status", %{
    ...
    :ok =
      Emitter.configure(:ovcs, "twelve_volt_battery_status", %{
        parameters_builder_function: :default,
        initial_data: %{
  ...
  def handle_info(%Bus.Message{name: :pack_current, value: pack_current, source: source}, state)
      when source == state.bms_status_source do
    {:noreply,
     %{
       state
       | pack_current: pack_current,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update || pack_current != state.pack_current
     }}
  end

  def handle_info(%Bus.Message{name: :pack_voltage, value: pack_voltage, source: source}, state)
      when source == state.bms_status_source do
    {:noreply,
     %{
       state
       | pack_voltage: pack_voltage,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update || pack_voltage != state.pack_voltage
     }}
  end

  def handle_info(
        %Bus.Message{name: :pack_state_of_charge, value: pack_state_of_charge, source: source},
        state
      )
      when source == state.bms_status_source do
    {:noreply,
     %{
       ...
       | pack_state_of_charge: pack_state_of_charge,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update ||
             pack_state_of_charge != state.pack_state_of_charge
     }}
  end

  def handle_info(
        %Bus.Message{name: :j1772_plug_state, value: j1772_plug_state, source: source},
        state
      )
      when source == state.bms_status_source do
    {:noreply,
     %{
       ...
       | j1772_plug_state: j1772_plug_state,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update || j1772_plug_state != state.j1772_plug_state
     }}
  end
  ...
  def handle_info(
        %Bus.Message{
          ...
          value: pack_average_temperature,
          source: source
        },
        state
      )
  ...
  def handle_info(
        %Bus.Message{
          ...
          value: charge_interlock_enabled,
          source: source
        },
        state
      )
  ...
  def handle_info(
        %Bus.Message{name: :balancing_active, value: balancing_active, source: source},
        state
      )
      when source == state.bms_status_source do
    {:noreply,
     %{
       ...
       | balancing_active: balancing_active,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update || balancing_active != state.balancing_active
     }}
  end
  ...
  def handle_info(
        %Bus.Message{
          ...
          value: charger_safety_relay_enabled,
          source: source
        },
        state
      )
    ...
    {:noreply,
     %{
       ...
       | charger_safety_relay_enabled: charger_safety_relay_enabled,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update ||
             charger_safety_relay_enabled != state.charger_safety_relay_enabled
     }}
  end

  def handle_info(%Bus.Message{name: :bms_error, value: bms_error, source: source}, state)
      when source == state.bms_status_source do
    {:noreply,
     %{
       state
       | bms_error: bms_error,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update || bms_error != state.bms_error
     }}
  end

  def handle_info(%Bus.Message{name: :is_alive, value: bms_is_alive, source: source}, state)
      when source == state.bms_status_source do
    {:noreply,
     %{
       state
       | bms_is_alive: bms_is_alive,
         pack_status_frame_requires_update:
           state.pack_status_frame_requires_update || bms_is_alive != state.bms_is_alive
     }}
  end

  def handle_info(
        %Bus.Message{
          name: :twelve_volt_battery_voltage,
          value: twelve_volt_battery_voltage,
          source: source
        },
        state
      )
    ...
    {:noreply,
     %{
       ...
       | twelve_volt_battery_voltage: twelve_volt_battery_voltage,
         twelve_volt_battery_status_frame_requires_update:
           state.twelve_volt_battery_status_frame_requires_update ||
             twelve_volt_battery_voltage != state.twelve_volt_battery_voltage
     }}
  end
  ...
  defp handle_pack_status(state) do
    case state.pack_status_frame_requires_update do
      true ->
        :ok =
          Emitter.update(:ovcs, "pack_status", fn data ->
            %{
              data
              | "pack_current" => state.pack_current,
                ...
                "pack_average_temperature" => state.pack_average_temperature,
                "is_charging" =>
                  state.charger_safety_relay_enabled && D.lt?(state.pack_current, @zero),
                "charge_interlock_enabled" => state.charge_interlock_enabled,
  ...
  defp handle_twelve_volt_battery_status_status(state) do
    case state.twelve_volt_battery_status_frame_requires_update do
      true ->
        :ok =
          Emitter.update(:ovcs, "twelve_volt_battery_status", fn data ->
            %{data | "battery_voltage" => state.twelve_volt_battery_voltage}
          end)
