defmodule VmsCore.Components.Volkswagen.Polo9N.PassengerCompartment do
  ...
  def handle_info(:loop, state) do
    Bus.broadcast("messages", %Bus.Message{
      ...
      value: state.front_left_door_open,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :front_right_door_open,
      value: state.front_right_door_open,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :rear_left_door_open,
      value: state.rear_left_door_open,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :rear_right_door_open,
      value: state.rear_right_door_open,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :trunk_door_open,
      value: state.trunk_door_open,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :beam_active,
      value: state.beam_active,
      source: __MODULE__
    })

    Bus.broadcast("messages", %Bus.Message{
      name: :handbrake_engaged,
      value: state.handbrake_engaged,
      source: __MODULE__
    })
