defmodule VmsCore.Managers.Gear do
  ...
  def init(%{
    ...
    :ok =
      Emitter.configure(:ovcs, "gear_status", %{
  ...
  def handle_info(
        %Bus.Message{name: :requested_gear_source, value: requested_gear_source, source: source},
        state
      )
  ...
  def handle_info(
        %Bus.Message{
          ...
          value: requested_direction_source,
          source: source
        },
        state
      )
  ...
  def handle_info(
        %Bus.Message{name: :requested_gear, value: requested_gear, source: source},
        state
      )
      when source == state.requested_gear_source and is_nil(state.requested_direction_source) do
    {:noreply, %{state | requested_gear: requested_gear}}
  end

  def handle_info(
        %Bus.Message{name: :requested_direction, value: requested_direction, source: source},
        state
      )
      when source == state.requested_direction_source and is_nil(state.requested_gear_source) do
    requested_gear =
      case requested_direction do
        :forward -> :drive
        :backward -> :reverse
      end
  ...
  defp select_gear(state) do
    ...
    cond do
      ...
      selected_gear != :parking && requested_gear == :parking && speed_near_zero &&
          throttle_near_zero ->
        apply_gear(state, :parking)
      ...
      selected_gear != :reverse && requested_gear == :reverse && speed_near_zero &&
        throttle_near_zero && ready_to_drive ->
        apply_gear(state, :reverse)

      selected_gear != :drive && requested_gear == :drive && speed_near_zero && throttle_near_zero &&
          ready_to_drive ->
  ...
  defp apply_gear(state, gear) do
    :ok =
      Emitter.update(:ovcs, "gear_status", fn data ->
