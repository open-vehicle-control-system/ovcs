  ...
  defp wait_for_interface(network_name, spi_interface)
    ...
    case match do
      [] ->
        ...
        receive do
          %PropertyTable.Event{
            table: NervesUEvent,
            property: _,
            value: %{"ifindex" => _, "interface" => can_interface, "subsystem" => "net"}
          } ->
            Logger.info(
              "SPI interface #{spi_interface} is ready and assigned to: #{network_name} => #{can_interface}"
            )
        ...
        after
          5000 ->
            case File.ls("/sys/bus/spi/devices/#{spi_interface}/net") do
              {:ok, [can_interface]} ->
                Logger.info(
                  "SPI interface #{spi_interface} is ready according to /sys/bus/spi/devices/ and assigned to: #{network_name} => #{can_interface}"
                )
              ...
              _ ->
                throw(
                  "SPI interface  '#{spi_interface}' to be used for '#{network_name}' not ready within 5 seconds, aborting!"
                )
            end
        end
      ...
      [{_, %{"interface" => existing_can_interface}}] ->
        Logger.info(
          "SPI interface #{spi_interface} was already and assigned to: #{network_name} => #{existing_can_interface}"
        )

        {network_name, existing_can_interface, labels: [spi_interface: spi_interface]}
    end
  end
